...
class res_pertner(models.Model):
    ...
    balance_invoice_ids = fields.One2many('account.move', 'partner_id', 'Customer move lines', domain=[
        ('move_type', 'in', ['out_invoice', 'out_refund', 'in_invoice', 'in_refund']),
        ('payment_state', 'not in', ['paid']), ('state', '=', 'posted')], groups="account.group_account_invoice")
        ...
        for vals in vals_list:
            ...
            if partner_id.Blocking_limit != 0.0:
                if partner_id.Blocking_limit <= partner_id.customer_due_amt:
                    raise UserError(
                        _('The Customer is in blocking stage and has to pay ' + str(partner_id.customer_due_amt)))

        result = super(sale_order, self).create(vals_list)
        return result
            ...
            if self.is_confirm == True:
                ...
                if partner.is_company:
                    account_move_line = account_move_line. \
                        sudo().search([('partner_id', '=', partner.id),
                ...
                for order in self:
                    ...
                    if self.partner_id.credit_on_hold is False:
                        ...
                        raise UserError(
                            _('You have been put on hold due to exceeding your credit limit. Please contact administration for further guidance. \n Thank You'))
                    return False
