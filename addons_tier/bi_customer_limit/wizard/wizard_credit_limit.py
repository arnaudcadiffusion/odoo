...
class wizard_credit_limit(models.TransientModel):
    ...
    _description = "Wizard Custom Credit"
    ...
    def confirm_sale(self):
        ...
        for partner in partner_id:
            if partner:
                template_id = self.env['ir.model.data'].sudo()._xmlid_lookup('bi_customer_limit.email_template_edi_credit_limit')[1]
                email_template_obj = self.env['mail.template'].sudo().browse(template_id)
                if template_id:
                    values = email_template_obj._generate_template(active_ids, ('subject', 'body_html', 'email_from', 'email_to', 'partner_to', 'email_cc', 'reply_to', 'scheduled_date'),)[active_ids [0]]
                    email_values = {}
