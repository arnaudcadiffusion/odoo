...
class PurchaseOrder(models.Model):
    ...
    cad_n_fournisseur = fields.Char(
        string='N# Fournisseur',
        related='partner_id.ref',
        store=True,
    )
    cad_date_depart = fields.Date(string='ETD')
    cad_date_prevue_initiale = fields.Date(string='ETD initiale')
    cad_dispo_confirmee = fields.Date(string='Dispo confirmée')
    cad_embarquement = fields.Boolean(string='Embarquement', default=False)
    cad_pol_1 = fields.Selection(
        selection=[
            ('SHANGHAI', 'SHANGHAI'),
            ('WUHAN', 'WUHAN'),
            ('QINGDAO', 'QINGDAO'),
    ...
    cad_transitaire = fields.Selection(
        selection=[
    ...
    cad_taux_eur_usd = fields.Float(string='Taux €/$')
    cad_fret = fields.Integer(string='Fret', default=0)
    cad_transport_achat = fields.Integer(string='Douane', default=0)
    cad_transport_po = fields.Float(string='Transport PO')


class PurchaseOrderLine(models.Model):
    _inherit = 'purchase.order.line'

    cad_date_initiale = fields.Date(
        string='Date Initiale',
        related='order_id.cad_date_prevue_initiale',
        store=True,
        readonly=True,
    )
    cad_ref_fournisseur = fields.Char(
    ...
    def _onchange_round_qty_to_carton(self):
        """Same as sale.order.line: while the user edits the line, round
        the quantity up to the next full carton (CARTON DE 12, 30 → 36).
        Quantities written by code (replenishment, imports) are left
        untouched."""
        for line in self:
            ...
            if not per_carton or float_is_zero(
                    line.nb_carton * per_carton - pieces,
