...
class SaleOrderLine(models.Model):
    ...
    @api.depends("product_uom_qty", "product_packaging_id", "product_uom_id")
    ...
    def _mapping_invoice_delivery_by_qty(self,qty_to_invoice):
        ...
        for move in moves:
            todo_moves |= move
            if sum(todo_moves.mapped('quantity')) == qty_to_invoice:
...
class SaleOrder(models.Model):
    ...
    def _prepare_purchase_order_line_data(self, so_line, date_order, company):
        ...
        result["product_uom_id"] = so_line.product_packaging_id.id
        result["product_uom_qty"] = so_line.product_packaging_qty
        return result
